# Teradja Mitchell
GRC and IAM analyst with hands-on experience across Microsoft Entra ID, Intune, Azure, and Microsoft 365 in regulated environments.
I build labs documenting real-world Microsoft Entra ID configurations on a live free-tier tenant. Identity lifecycle management, group-based access control, role assignments, app registrations, B2B collaboration, and more. Every step validated in the portal and via PowerShell.
This is where I show the work.

## Stack
**Identity and Access Management**
Microsoft Entra ID · Conditional Access · MFA · RBAC · Privileged Identity Management (PIM) · Access Reviews · App Registrations · SSO · Microsoft Graph API · PowerShell · Zero Trust Architecture

**Microsoft 365 Security**
Microsoft Defender XDR · Defender for Endpoint · Defender for Office 365 · Microsoft Purview · Exchange Online · SharePoint Online · Microsoft Teams · Secure Score

**Endpoint and Device Engineering**
Microsoft Intune · MDM/MAM · Device Compliance Policies · Configuration Profiles · Attack Surface Reduction (ASR) · BitLocker · Windows Update for Business · Application Packaging (.intunewin)

**Cloud and Infrastructure**
Microsoft Azure · Azure Firewall · Azure Virtual Desktop (AVD) · Azure RBAC · Network Security Groups

**Governance, Risk and Compliance**
HIPAA · SOC 2 Type II · FedRAMP · HITRUST · NIST 800-53 · NIST CSF · NIST RMF · ISO 27001 · Vanta

**Automation and Tooling**
PowerShell · Microsoft Graph API · Vanta · AI-assisted security workflows

## Certs
- CompTIA SecurityX (CASP+)
- CompTIA Security+
- SC-300 Microsoft Identity and Access Administrator *(in progress)*

## Labs
Real configurations. Step-by-step. Compliance context included.

| # | Lab | Maps To |
|---|-----|---------|
| 01 | [MFA Enforcement Per-User Multifactor Authentication](https://github.com/teradja757/entra-id-labs/tree/main/labs/lab-01-mfa-enforcement) | HIPAA · SOC 2 · HITRUST |
| 02 | [Security Group Creation and Membership Management](https://github.com/teradja757/entra-id-labs/tree/main/labs/lab-02-security-group-management) | HIPAA · SOC 2 · HITRUST · NIST 800-53 |
| 03 | [Entra ID Role Assignment and Privileged Access Management](https://github.com/teradja757/entra-id-labs/tree/main/labs/lab-03-role-assignment-and-pim) | HIPAA · SOC 2 · HITRUST · NIST 800-53 |
| 04 | [App Registrations and Role-Based Access Control](https://github.com/teradja757/entra-id-labs/tree/main/labs/lab-04-app-registrations-and-rbac) | HIPAA · SOC 2 · HITRUST · NIST 800-53 |
| 05 | [Guest User Access and B2B Collaboration](https://github.com/teradja757/entra-id-labs/tree/main/labs/lab-05-guest-user-access-b2b) | HIPAA · SOC 2 · HITRUST · NIST 800-53 |
| 06 | [SSO Token Configuration and OIDC Claim Mapping](https://github.com/teradja757/entra-id-labs/tree/main/labs/lab-06-sso-oidc-token-configuration) | HIPAA · SOC 2 · HITRUST · NIST 800-53 |

*More to come.*

[LinkedIn](https://www.linkedin.com/in/teradja-mitchell-0494b9208)
